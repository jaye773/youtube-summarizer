...
class TestVoiceAPIEndpoints(VoiceValidationTestCase):
    ...
    def test_preview_voice_endpoint(self, mock_tts_client):
        ...
        payload = {"voice_id": "en-US-Chirp3-HD-Zephyr", "text": "Test preview text"}

        response = self.client.post("/preview-voice", data=json.dumps(payload), content_type="application/json")
    ...
    def test_preview_voice_invalid_voice_id(self):
        ...
        payload = {"voice_id": "invalid-voice-id", "text": "Test text"}
    ...
    def test_preview_voice_missing_params(self):
        ...
        response = self.client.post(
            "/preview-voice", data=json.dumps({"text": "Test"}), content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)

        # Missing text
        response = self.client.post(
            "/preview-voice", data=json.dumps({"voice_id": "test"}), content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_preview_voice_text_length_limit(self):
        ...
        with patch("app.tts_client") as mock_tts_client:
            ...
            payload = {"voice_id": "en-US-Chirp3-HD-Zephyr", "text": long_text}
    ...
    def test_speak_endpoint_with_voice_selection(self, mock_tts_client):
        ...
        payload = {"text": "Test summary text", "voice_id": "en-US-Neural2-J"}

        response = self.client.post("/speak", data=json.dumps(payload), content_type="application/json")
...
class TestSettingsIntegration(VoiceValidationTestCase):
    ...
    def test_settings_update_voice_persistence(self):
        ...
        payload = {"tts_voice": new_voice, "google_api_key": "test_key"}

        response = self.client.post("/settings", data=json.dumps(payload), content_type="application/json")
    ...
    def test_settings_invalid_voice_fallback(self):
        ...
        payload = {"tts_voice": invalid_voice, "google_api_key": "test_key"}
...
class TestUserExperienceFlow(VoiceValidationTestCase):
    ...
    def test_voice_selection_to_summary_playback(self):
        ...
        settings_response = self.client.post(
            "/settings", data=json.dumps(settings_payload), content_type="application/json"
        )
        ...
        with patch("app.tts_client") as mock_tts_client:
            ...
            speak_response = self.client.post("/speak", data=json.dumps(speak_payload), content_type="application/json")
...
class TestEdgeCasesAndErrorHandling(VoiceValidationTestCase):
    ...
    def test_empty_or_malformed_requests(self):
        ...
        # Empty JSON
        response = self.client.post("/preview-voice", data="{}", content_type="application/json")
        self.assertEqual(response.status_code, 400)

        # Malformed JSON
        response = self.client.post("/preview-voice", data="invalid json", content_type="application/json")
        self.assertEqual(response.status_code, 400)

        # No content type
        response = self.client.post("/preview-voice", data=json.dumps({"voice_id": "test", "text": "test"}))
    ...
    def test_very_long_text_handling(self):
        ...
        with patch("app.tts_client") as mock_tts_client:
            ...
            preview_payload = {"voice_id": "en-US-Chirp3-HD-Zephyr", "text": very_long_text}

            response = self.client.post(
                "/preview-voice", data=json.dumps(preview_payload), content_type="application/json"
            )
            ...
            response = self.client.post("/speak", data=json.dumps(speak_payload), content_type="application/json")
...
class TestPerformanceAndReliability(VoiceValidationTestCase):
    ...
    def test_concurrent_voice_requests(self, mock_tts_client):
        ...
        def make_request():
            payload = {"voice_id": "en-US-Chirp3-HD-Zephyr", "text": "Concurrent test"}
            response = self.client.post("/preview-voice", data=json.dumps(payload), content_type="application/json")
