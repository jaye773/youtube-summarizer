...
convert_single_video = 0 #set to 0 for a playlist, set to 1 for a video
...
single_video_url = 'https://www.youtube.com/watch?v=kOXJth7z4NU&list=PL0dTxWJ6ngULGVMouT4eNBrqvtwZNJ2yX&index=26&pp=iAQB'
playlist_url = 'https://youtube.com/playlist?list=PL0dTxWJ6ngULGVMouT4eNBrqvtwZNJ2yX&si=j16i-A8VjhTOsiKF'

# Auto-save settings
AUTO_SAVE_INTERVAL = 5  # Save every 10 segments processed
PARTIAL_SAVE_DIR = "partial_transcripts"
...
os.makedirs(PARTIAL_SAVE_DIR, exist_ok=True)  # Create directory for partial saves
...
def save_partial_transcript(audio_file_name, segments_processed, combined_transcript_text, 
                          combined_transcript_text_list_of_metadata_dicts, list_of_transcript_sentences):
    """Save partial transcript progress to prevent data loss"""
    partial_save_path = os.path.join(PARTIAL_SAVE_DIR, f"{audio_file_name}_partial.json")
    
    partial_data = {
        'audio_file_name': audio_file_name,
        'segments_processed': segments_processed,
        'timestamp': datetime.datetime.now(datetime.UTC).isoformat(),
        'combined_text': combined_transcript_text,
        'metadata': combined_transcript_text_list_of_metadata_dicts,
    ...
    try:
        ...
        print(f"✓ Auto-saved partial transcript: {segments_processed} segments processed")
    except Exception as e:
        print(f"⚠️  Could not save partial transcript: {e}")

def load_partial_transcript(audio_file_name):
    """Load existing partial transcript if available"""
    partial_save_path = os.path.join(PARTIAL_SAVE_DIR, f"{audio_file_name}_partial.json")
    ...
    if os.path.exists(partial_save_path):
        try:
            with open(partial_save_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            print(f"🔄 Resuming from partial transcript: {data['segments_processed']} segments already processed")
            return data
        except Exception as e:
            print(f"⚠️  Could not load partial transcript: {e}")
...
async def compute_transcript_with_whisper_from_audio_func(audio_file_path, audio_file_name, audio_file_size_mb):
    ...
    # Check for existing partial transcript
    partial_data = load_partial_transcript(audio_file_name)
    if partial_data:
        combined_transcript_text = partial_data['combined_text']
        combined_transcript_text_list_of_metadata_dicts = partial_data['metadata']
        list_of_transcript_sentences = partial_data['sentences']
        segments_processed = partial_data['segments_processed']
        print(f"Resuming from segment {segments_processed}")
    else:
    ...
    if use_openai_api_for_transcription:
        ...
        with tqdm(total=audio_duration_seconds, desc=f"Transcribing {audio_file_name}", unit="s") as pbar:
            ...
            for i, segment in enumerate(segments):
                # Skip segments that were already processed
                if i < segments_processed:
                ...
                # Auto-save every AUTO_SAVE_INTERVAL segments
    ...
    with open(f'generated_transcript_combined_texts/{audio_file_name}.txt', 'w',  encoding="utf-8") as file:
    ...
    if os.path.exists(partial_save_path):
        try:
            ...
            print(f"✓ Cleaned up partial transcript file for {audio_file_name}")
        except Exception as e:
            print(f"⚠️  Could not clean up partial transcript file: {e}")
