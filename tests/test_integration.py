...
class TestIntegration(unittest.TestCase):
    ...
    def test_full_video_summarization_flow(self, mock_model, mock_transcript_api, mock_youtube):
        ...
        response = self.client.post('/summarize',
                                   data=json.dumps({'urls': [video_url]}),
    ...
    @patch('app.youtube')
    @patch('app.summary_cache')
    def test_playlist_summarization_flow(self, mock_cache, mock_youtube):
        """Test playlist summarization flow"""
        playlist_url = "https://www.youtube.com/playlist?list=PLrAXtmErZgOeiKm4sgNOknGvNjby9efdf"
        playlist_id = "PLrAXtmErZgOeiKm4sgNOknGvNjby9efdf"
        
        # Mock playlist metadata
        pl_meta_request = MagicMock()
        pl_meta_request.execute.return_value = {
            'items': [{
        ...
        # Setup mock returns
        mock_youtube.playlists().list.return_value = pl_meta_request
        mock_youtube.playlistItems().list.return_value = pl_items_request
        
        # Mock cache for video1
        mock_cache.__contains__.side_effect = lambda x: x == 'video1'
        mock_cache.__getitem__.side_effect = lambda x: {
            'video1': {
                'title': 'Video 1',
                'summary': 'Cached summary for video 1',
        ...
        # Make request
        response = self.client.post('/summarize',
                                   data=json.dumps({'urls': [playlist_url]}),
                                   content_type='application/json')
    ...
    def test_invalid_url_handling(self):
        ...
        response = self.client.post('/summarize',
                                   data=json.dumps({'urls': invalid_urls}),
                                   content_type='application/json')
