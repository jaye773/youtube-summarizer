...
def mock_youtube_api():
    ...
    with patch("app.get_video_details") as mock_get_video, patch("app.get_transcript") as mock_get_transcript, patch(
        "app.get_videos_from_playlist"
...
class TestCompleteVideoWorkflow:
    ...
    def test_video_processing_success_workflow(self, client, full_worker_system, mock_youtube_api, mock_ai_providers):
        ...
        # Step 1: Submit job
        response = client.post("/summarize_async", json=job_data, content_type="application/json")
    ...
    def test_video_processing_with_error_recovery(
        ...
        job_data = {"url": "https://www.youtube.com/watch?v=error123", "ai_provider": "openai", "model": "gpt-4o"}
        ...
        job.progress = 100
        job.result = JobResult(
            job_id=job_id, status=JobStatus.COMPLETED, result={"summary": "Retry successful", "video_id": "error123"}
        )
    ...
    def test_multiple_concurrent_jobs(self, client, full_worker_system, mock_youtube_api, mock_ai_providers):
        ...
        for i, job_id in enumerate(job_ids):
            ...
            job.result = JobResult(
                job_id=job_id,
                status=JobStatus.COMPLETED,
                result={"summary": f"Concurrent job {i + 1} summary", "video_id": f"concurrent{i}"},
            )
...
class TestProgressTrackingAndNotifications:
    ...
    def test_sse_progress_notifications(self, client, full_worker_system):
        ...
        for progress, step in progress_stages:
            ...
            if progress == 100:
                job.status = JobStatus.COMPLETED
                job.result = JobResult(
                    job_id=job_id, status=JobStatus.COMPLETED, result={"summary": "Progress test complete"}
                )
    ...
    def test_job_status_polling(self, client, full_worker_system):
        ...
        job.status = JobStatus.COMPLETED
        job.progress = 100
        job.result = JobResult(job_id=job_id, status=JobStatus.COMPLETED, result={"summary": "Polling test complete"})
...
class TestSystemIntegrationScenarios:
    ...
    def test_mixed_job_types_workflow(self, client, full_worker_system, mock_youtube_api, mock_ai_providers):
        ...
        video_job_obj.result = JobResult(
            job_id=video_job_id, status=JobStatus.COMPLETED, result={"summary": "Mixed video summary", "type": "video"}
        )
